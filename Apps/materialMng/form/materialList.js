...
export default async function (_Context,container) {
    ...
    const _fileListElm = _rootElm.querySelector('.w3-container.list');

    container.appendChild(_rootElm);
    
    _fileListElm.style.width = '320px';
    ...
    const _listElm = _rootElm.querySelector('ul.list');
    ...
    async function _updateList() {
        ...
        if (res.r === 'ok') {
            ...
            _list.forEach(item => {
                ...
                li.dataset.data = JSON.stringify(item);
